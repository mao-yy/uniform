...
class TestMeter(object):
    ...
    def log_iter_stats(self, cur_iter):
        ...
        stats = {
            ...
            "cur_iter": "{}".format(cur_iter + 1),
            "eta": eta,
            "time_diff": "{:.5f}".format(self.iter_timer.seconds()),
        }
        inf_progress(cur_iter+1,self.overall_iters,
                    prefix= "",
                    suffix=json.dumps(stats))
        # logging.log_json_stats(stats)
    ...
    def finalize_metrics(self, ks=(1)):
        ...
        if self.multi_label:
            ...
            pred_softmax=self.video_preds.cpu().numpy().argmax(1)
            label=self.video_labels.cpu().numpy()
            self.stats["acc"]=np.mean(pred_softmax==label)
...
class TrainMeter(object):
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        stats = {
            ...
            # "dt_net": self.net_timer.seconds(),
            "eta": eta,
            "loss": f"%.2f"%self.loss.get_win_median(),
            "lr": f"%.6f"%self.lr,
            # "gpu_mem": "{:.2f}G".format(misc.gpu_mem_usage()),
        }
        if not self._cfg.DATA.MULTI_LABEL:
        ...
        # inf_progress(cur_iter+1,self.epoch_iters,
        #             prefix= "epoch {}/{}".format(cur_epoch + 1, self._cfg.SOLVER.MAX_EPOCH),
        #             suffix=json.dumps(stats))
        # logging.log_json_stats(stats)
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        stats = {
            ...
            "dt": "{:.2}".format(self.iter_timer.seconds()) ,
            "dt_data":  "{:.2}".format(self.data_timer.seconds()) ,
            "dt_net":  "{:.2}".format(self.net_timer.seconds()),
            "eta": eta,
            "lr":  "{:.6}".format(self.lr),
            "gpu_mem": "{:.2f}G".format(misc.gpu_mem_usage()),
            "RAM": "{:.2f}/{:.2f}G".format(*misc.cpu_mem_usage()),
        ...
        if not self._cfg.DATA.MULTI_LABEL:
            ...
            stats["top1_err"] =  "%.2f"%(top1_err) 
            stats["top5_err"] = "%.2f"%(top5_err) 
            stats["loss"] = "%.5f"%(avg_loss) 
        logging.log_json_stats(stats)
...
class ValMeter(object):
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        if not self._cfg.DATA.MULTI_LABEL:
            pass
            # stats["top1_err"] =f"%.4"%( self.mb_top1_err.get_win_median())
            # stats["top5_err"] = f"%.4"%( self.mb_top5_err.get_win_median())
        # logging.log_json_stats(stats)
        # inf_progress(cur_iter+1,self.max_iter,
        #             prefix= "epoch {}/{}".format(cur_epoch + 1, self._cfg.SOLVER.MAX_EPOCH),
        #             suffix=json.dumps(stats))
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        stats = {
            "_type": "val_epoch",
            "epoch": "{}/{}".format(cur_epoch + 1, self._cfg.SOLVER.MAX_EPOCH),
            "time_diff": "{:.3f}".format(self.iter_timer.seconds()),
            "gpu_mem": "{:.2f}G".format(misc.gpu_mem_usage()),
            "RAM": "{:.2f}/{:.2f}G".format(*misc.cpu_mem_usage()),
        }
        if self._cfg.DATA.MULTI_LABEL:
            ...
            stats["top1_err"] = "%.2f"%(top1_err)
            stats["top5_err"] =  "%.2f"%(top5_err)
            stats["min_top1_err"] = "%.2f"%(self.min_top1_err)
            stats["min_top5_err"] = "%.2f"%(self.min_top5_err)
